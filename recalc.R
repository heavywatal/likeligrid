...
.tablize = function(.data) {
    tibble(genotype= .env$.data[['sample']]) %>%
      dplyr::mutate(
...
.cal_loglik = function(.data, .th_path) {
    .tbl = .tablize(.data)
    .w_gene = .tbl %>% dplyr::summarise_at(vars(matches('g_\\w\\d')), sum) %>% {. / sum(.)} %>% print()
...
.cal_loglik(.testcase, .th_path) %>% print() %>% {sum(.$loglik)}
.cal_loglik(.s3, .th_path) %>% print() %>% {sum(.$loglik)}
