...
- genotypes ($I \times J$ matrix)
    ...
    - number of non-new mutations: $a_{i,j} = \max(0, s_{i,j} - 1)$
...
- sample (row) $i$
    ...
    - unnormalized probability of the configuration:
      ...
      = S^{-s_i} M_i \prod_j s_j^{s_{i,j}} \theta_j^{a_{i,j}}$
    - denominator to normalize probability:
      $D_s(\theta) = \sum_{k \in K \mid s_k = s} p_k(\theta)
      = S^{-s} \sum_{k \in K \mid s_k = s} M_k \prod_j s_j^{s_{k,j}} \theta_j^{a_{k,j}}$
...
l(\theta)
  ...
  &= \sum_i \left( \log M_i \prod_i s_j^{s_{i,j}} \theta_j^{a_{i,j}} \right)
   - \sum_i \log S^{s_i} D_{s_i}(\theta) \\
  &= \sum_i \left(\log M_i + \sum_j s_{i,j} \log s_j + \sum_j a_{i,j} \log \theta_j \right)
   - \sum_s N_s \log S^s D_s(\theta) \\
  &= \sum_i \log M_i
   + \sum_j s_j \log s_j
   + \sum_j a_j \log \theta_j
   - \sum_s \left(N_s \log \sum_{k \in K \mid s_k = s} M_k \prod_j s_j^{s_{k,j}} \theta_j^{a_{k,j}} \right) \\
\end{split}$$</div>
...
l(\theta)
  ...
  &= \sum_i \left(\log M_i + \sum_j s_{i,j} \log w_j + \sum_j a_{i,j} \log \theta_j \right)
   - \sum_s N_s \log D_s(\theta) \\
  &= \sum_i \log M_i
   + \sum_j s_j \log w_j
   + \sum_j a_j \log \theta_j
   - \sum_s \left(N_s \log \sum_{k \in K \mid s_k = s} M_k \prod_j w_j^{s_{k,j}} \theta_j^{a_{k,j}} \right) \\
\end{split}$$</div>
